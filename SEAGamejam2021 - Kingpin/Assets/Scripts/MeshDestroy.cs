...
public class MeshDestroy : MonoBehaviour
{
    ...
    [SerializeField] int maximumCutPieces = 10;
    [SerializeField] int cutPieceMultiplier = 2;
    [SerializeField] int destructableThreshold = 10;
    [SerializeField] float ExplodeForce = 100;
    [SerializeField] int destructionLayer = 16;

    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.GetComponent<PlayerHandCollider>() != null)
        {
            if (collision.gameObject.GetComponent<PlayerHandCollider>().currentMultiplierPower > destructableThreshold)
            {
    ...
    private void DestroyMesh()
    {
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            parts[i].MakeGameobject(this, destructionLayer);
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshDestroy original, int newObjectLayer)
        {
            GameObject = new GameObject(original.name);
            GameObject.transform.position = original.transform.position;
            GameObject.transform.rotation = original.transform.rotation;
            GameObject.transform.localScale = original.transform.localScale;
            GameObject.gameObject.layer = newObjectLayer;
